...
Select 
    ...
    grado
    FROM centri_agenti
    INNER JOIN centro ON centri_agenti.fk_centro = centro.id
    INNER JOIN agente ON centri_agenti.fk_agente = agente.id
    WHERE agente.attivo = b'1';

Select agente.id, agente.nome, centro.id, centro.nome, contratto.tipo, contratto.tariffa
    FROM contratto
    INNER JOIN centro ON contratto.fk_centro = centro.id
    INNER JOIN agente ON contratto.fk_agente = agente.id
    WHERE agente.attivo = b'1';

Select agente.id, agente.nome, centro.id, centro.nome, contratto.tipo, contratto.tariffa, contratto.created_at
            FROM contratto
            INNER JOIN centro ON contratto.fk_centro = centro.id
            INNER JOIN agente ON contratto.fk_agente = agente.id
            WHERE agente.attivo = b'1' AND centro.id IN (1,2,3) ORDER BY Year(contratto.created_at) DESC,month(contratto.created_at) DESC, contratto.tipo DESC

SELECT agente.id, agente.nome, centro.id, centro.nome,grado 
	FROM centri_agenti
	INNER JOIN agente on agente.id = centri_agenti.fk_agente
	Inner JOIN centro on centro.id =  centri_agenti.fk_centro
	WHERE centro.id = 3 AND centri_agenti.grado = 10

SELECT agente.id, agente.nome, centro.id, centro.nome,grado FROM centri_agenti
INNER JOIN agente on agente.id = centri_agenti.fk_agente
Inner JOIN centro on centro.id =  centri_agenti.fk_centro
WHERE agente.id = 1 AND centri_agenti.grado = 10

Select agente.id, agente.nome, centro.id, centro.nome, contratto.tipo, contratto.tariffa, contratto.created_at
            FROM contratto
            INNER JOIN centro ON contratto.fk_centro = centro.id
            INNER JOIN agente ON contratto.fk_agente = agente.id
            WHERE agente.attivo = b'1' AND centro.id IN (SELECT centro.id FROM centri_agenti
                                                            INNER JOIN agente on agente.id = centri_agenti.fk_agente
                                                            Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                                                            WHERE agente.id IN (SELECT agente.id
                                                                                    FROM centri_agenti
                                                                                    INNER JOIN agente on agente.id = centri_agenti.fk_agente
                                                                                    Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                                                                                    WHERE centro.id = 3 AND centri_agenti.grado = 10 AND centri_agenti.grado = 10)AND centri_agenti.grado = 10
                                                            ) ORDER BY Year(contratto.created_at) DESC,month(contratto.created_at) DESC, contratto.tipo DESC





Select COUNT(contratto.id)
            FROM contratto
            INNER JOIN centro ON contratto.fk_centro = centro.id
            INNER JOIN agente ON contratto.fk_agente = agente.id
            WHERE agente.attivo = b'1'  AND 
            EXTRACT(MONTH FROM contratto.created_at) = EXTRACT(MONTH FROM CURRENT_TIMESTAMP) AND EXTRACT(YEAR FROM contratto.created_at) = EXTRACT(YEAR FROM CURRENT_TIMESTAMP)  AND
                centro.id IN (SELECT centro.id FROM centri_agenti
                                INNER JOIN agente on agente.id = centri_agenti.fk_agente
                                Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                                WHERE agente.id IN (SELECT agente.id
                                                        FROM centri_agenti
                                                        INNER JOIN agente on agente.id = centri_agenti.fk_agente
                                                        Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                                                        WHERE centro.id = 2 AND centri_agenti.grado = 10) AND centri_agenti.grado >= 10
                                ) 

Select agente.id, agente.nome, centro.id, centro.nome, agente.created_at
        FROM agente
        INNER JOIN centri_agenti ON centri_agenti.fk_agente = agente.id
        INNER JOIN centro ON centri_agenti.fk_centro = centro.id
        WHERE agente.id != 0 AND
            agente.id != 1 AND
            centri_agenti.attivo != b'0' AND
            LOCATE('M',agente.nome) > 0 AND 
            centro.nome != ""AND
            centro.id IN (SELECT centro.id FROM centri_agenti
                            INNER JOIN agente on agente.id = centri_agenti.fk_agente
                            Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                            WHERE agente.id IN (SELECT agente.id
                                                    ...
                                                    Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                                                    WHERE centro.id = 2 AND centri_agenti.grado = 10) AND centri_agenti.grado >= 10
                            ) ORDER BY agente.id, Year(agente.created_at) DESC,month(agente.created_at) DESC;


Select agente.nome, COUNT(contratto.id) maximum
        FROM contratto
        INNER JOIN agente ON contratto.fk_agente = agente.id
        INNER JOIN centro ON contratto.fk_centro = centro.id
        WHERE  contratto.fk_centro IN (SELECT centro.id FROM centri_agenti
                            INNER JOIN agente on agente.id = centri_agenti.fk_agente
                            Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                            WHERE agente.id IN (SELECT agente.id
                                                    FROM centri_agenti
                                                    ...
                                                    Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                                                    WHERE centro.id = 2 AND centri_agenti.grado = 10) AND centri_agenti.grado >= 10
                            ) GROUP BY contratto.fk_agente ORDER BY maximum DESC LIMIT 1;


//prendi il nome del centro
Select centro.nome, COUNT(contratto.id)
                FROM contratto
                INNER JOIN centro ON contratto.fk_centro = centro.id
                INNER JOIN agente ON contratto.fk_agente = agente.id
                WHERE 
                EXTRACT(MONTH FROM contratto.created_at) = EXTRACT(MONTH FROM CURRENT_TIMESTAMP) AND EXTRACT(YEAR FROM contratto.created_at) = EXTRACT(YEAR FROM CURRENT_TIMESTAMP)  AND
                    ...
                    INNER JOIN agente on agente.id = centri_agenti.fk_agente
                    Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                    WHERE centro.id = 1 AND centri_agenti.grado = 10) AND centri_agenti.grado >= 10) GROUP BY contratto.fk_centro;



SELECT giorno, SUM(numero)
FROM
(
    SELECT (current_date) giorno,0 as numero UNION
SELECT(current_date - interval 1 day)giorno,0  as numero UNION
SELECT(current_date - interval 2 day)giorno,0 as numero  UNION
SELECT(current_date - interval 3 day)giorno,0  as numero UNION
SELECT(current_date - interval 4 day)giorno,0 as numero  UNION
SELECT(current_date - interval 5 day)giorno,0 as numero  UNION
SELECT(current_date - interval 6 day)giorno,0 as numero  UNION
SELECT(current_date - interval 7 day)giorno,0  as numero 

UNION
SELECT DATE(contratto.created_at)giorno, count(contratto.id)numero from contratto

INNER JOIN AGENTE ON AGENTE.ID = CONTRATTO.fk_agente
INNER JOIN CENTRO ON CENTRO.ID = CONTRATTO.fk_centro
WHERE contratto.created_at > now() - interval 1 week AND
CONTRATTO.fk_centro IN (SELECT centri_agenti.fk_centro FROM centri_agenti 
                              
INNER JOIN AGENTE ON AGENTE.ID = centri_agenti.fk_agente
INNER JOIN CENTRO ON CENTRO.ID = centri_agenti.fk_centro
                             WHERE GRADO = 10 AND centri_agenti.fk_agente IN (SELECT centri_agenti.fk_agente FROM centri_agenti WHERE centri_agenti.fk_centro = 2 AND GRADO = 10)) GROUP BY DAY(contratto.created_at)
...
SELECT DAYOFWEEK(created_at) = 7
or DAYOFWEEK(created_at) = 1
or (DATE_FORMAT(created_at, "%T") > '08:30:00' AND DAYOFWEEK(created_at) = 6) 
or DATE(created_at) = '2022-03-03'

FROM contratto

SELECT DAYOFWEEK(CURRENT_DATE) = 7
or DAYOFWEEK(CURRENT_DATE) = 1
or (DATE_FORMAT(CURRENT_DATE, "%T") > '08:30:00' AND DAYOFWEEK(created_at) = 6) 
or DATE(CURRENT_DATE) = '2022-03-03' as "festivo"

FROM contratto LIMIT 1



SELECT  DAYOFWEEK(CURRENT_DATE) = 7
or DAYOFWEEK(created_at) = 1
or (DATE_FORMAT(created_at, "%T") > '08:30:00' AND DAYOFWEEK(CURRENT_DATE) = 6) 
or DATE(created_at) = '2022-03-03' as "festivo"

FROM contratto
inner join centro on centro.id = contratto.fk_centro
where date(created_at) in(select FROM_UNIXTIME(UNIX_TIMESTAMP(CONCAT(CONCAT(YEAR(CURRENT_DATE),"-",MONTH(CURRENT_DATE),"-"),n)),'%Y-%m-%d') as giorni from (
        ...
        where n > 0 and n <= day(CURRENT_DATE) 
...
SELECT centro.nome, centro.obiettivo_feriale,centro.obiettivo_festivo FROM centri_agenti
                INNER JOIN agente on agente.id = centri_agenti.fk_agente
                Inner JOIN centro on centro.id =  centri_agenti.fk_centro
                WHERE agente.id IN (SELECT agente.id
                FROM centri_agenti
